    ...
    def populate_aliment(self):
        ...
        while Product.objects.count() < nb_aliments_to_display:
            ...
            for product in data['products']: 
                ...
                try:
                    cat_list = product['categories'].split(',') # We put all the Product categories into a list
                    aliment = Product.objects.create(
                        ...
                        last_cat = cat_list[-1] # Will be used to find a substitut later
                    )

                    if len(cat_list) > 1: # If we have more than 1 cat we can add a fallback cat
                        aliment.fallback_cat = cat_list[-2]
                        aliment.save()
                        
                    print('Aliment suivant ajouté : ', product['product_name_fr'])
                    ...
                    print('Un aliment de ce nom est déjà present : ', product['product_name_fr'])
                    pass
